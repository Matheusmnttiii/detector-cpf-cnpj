...
function validateNow(){
  ...
  if(digits.length === 11){
    ...
    setResult(
      ...
      `${ok ? "Documento válido! " : "Documento inválido! "}Formato: ${formatCPF(digits)}`
    );
    return;
  }
  ...
  if(digits.length === 14){
    ...
    setResult(
      ...
      `${ok ? "Documento válido! " : "Documento inválido! "}Formato: ${formatCNPJ(digits)}`
    );
    return;
  }

  setResult("bad", "⚠️ Tamanho inválido", "CPF tem 11 dígitos e CNPJ tem 14 dígitos.");
}
...
btnCopy.addEventListener("click", async () => {
  ...
  try{
    ...
    setResult("bad", "❌ Erro ao copiar", "Seu navegador bloqueou. Copie manualmente.");
  }
});

btnAbout.addEventListener("click", () => {
  setResult("good", "ℹ️ ID Validator", "App para validação instantânea de CPF e CNPJ com formatação automática.");
});

// Exemplo cards
document.querySelectorAll(".example-card").forEach(card => {
  card.addEventListener("click", () => {
    const testNumber = card.getAttribute("data-test");
    elDoc.value = "";
    elDoc.focus();
    ...
    for(let i=0; i<testNumber.length; i++){
      setTimeout(() => {
        elDoc.value = testNumber.slice(0, i+1);
        autoFormat();
        const digits = onlyDigits(elDoc.value);
        if(digits.length === 11 || digits.length === 14) validateNow();
      }, i * 30);
